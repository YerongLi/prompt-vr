...
DATAROOT = os.getenv('HOME')
def process_vrd_split(in_split, out_split):
    vrd_dir = DATAROOT + '/data/vrd/sg_dataset/sg_' + in_split + '_images/'
    new_dir = DATAROOT + '/data/vrd/'+ out_split + '_images/'
    ...
    name_map_fname = DATAROOT + '/data/vrd/%s_fname_mapping.json' %(out_split)
    with open(name_map_fname, 'w') as f:
        json.dump(name_map, f, sort_keys=True, indent=4)
        f.close()

    # load the original annotations
    with open(DATAROOT + '/data/vrd/annotations_' + in_split + '.json', 'r') as f:
    ...
    with open(DATAROOT + '/data/vrd/new_annotations_' + out_split + '.json', 'w') as outfile:
