...
def convert_anno(split):
    ...
    with open(DATAROOT + '/data/vrd/new_annotations_' + split + '.json', 'r') as f:
        vrd_anns = json.load(f)


    print(len(vrd_anns))

    img_dir = DATAROOT + '/data/vrd/' + split + '_images/'
    new_imgs = []
    new_anns = []
    ann_id = 1
    ...
    with open(DATAROOT + '/data/vrd/objects.json', 'r') as f:
    ...
    with open(DATAROOT + '/data/vrd/detections_' + split + '.json', 'w') as outfile:
